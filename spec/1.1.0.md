...
A non-object `extensions` field is handled as defined in §8.1. Every permitted field otherwise MUST match the type and constraints defined below. Any manifest violation other than an unknown top-level field, a non-object `extensions` field, or a string `icon` value that violates only the plugin-relative path requirements of §5.8 is fatal: the client MUST reject the plugin and MUST NOT discover or execute any of its components.
...
| Field         | Type     | Description                                                           |
| ------------- | -------- | --------------------------------------------------------------------- |
| `displayName` | string   | Human-readable label for client UIs. See §5.7.                         |
...
The optional `displayName` field is a human-readable label for the plugin, intended for presentation in client user interfaces. Unlike `name`, it is not constrained by §5.5 and MAY contain mixed case, spaces, and other characters.

If present, `displayName` MUST be a string. A client that displays a plugin label MAY use `displayName` instead of `name`. If `displayName` is absent, the client MUST use `name` for that label. A client SHOULD treat an empty `displayName` as absent and fall back to `name`.

### 5.8 Icon

The optional `icon` field identifies an image that represents the plugin in client user interfaces. If present, `icon` MUST be a string.

The value is a plugin-relative path as defined in §4.1: it MUST begin with `./`, be resolved against the plugin root, and remain within the filesystem-resolved plugin root. Agent Plugins does not define remote (`http:` or `https:`) icon references, and a client MUST NOT retrieve a remote resource to resolve `icon`.

The image SHOULD be square and at least 512x512 pixels. Whether and how a client renders the icon is client-defined, and a client MAY ignore `icon` entirely.

`icon` is advisory. At the schema level, the only constraint on `icon` is that, if present, it MUST be a string; a non-string `icon` is a fatal type violation under §5.2. A string `icon` that does not satisfy the plugin-relative path requirements above is not a fatal violation: if `icon` is absent, is not a valid plugin-relative path, or resolves outside the plugin root, the client MUST ignore the field and MUST continue loading the plugin, and the value or validity of `icon` MUST NOT affect component discovery or execution. A client that renders the icon MUST treat the referenced file as untrusted image data: it MUST NOT execute or otherwise interpret any active content the file may contain (for example, script embedded in an SVG), and rendering the icon MUST NOT cause any network request, including a request to resolve a subresource referenced by the file. Agent Plugins defines no portable default icon; any placeholder is client-defined.
...
- [ ] When displaying a plugin label, prefer `displayName` when present and non-empty, otherwise fall back to `name` ([§5.7](#57-display-name))
...
A plugin's human-readable label and packaged visual identifier need to travel with the plugin for it to remain recognizable across clients. The constrained `name` identifier (§5.5) is unsuitable as a user-facing label, so `displayName` provides an unconstrained label with a defined fallback to `name`. `icon` provides an optional visual identifier. Both fields affect presentation only and never component discovery or execution.

The portable contract stops at identity. `icon` is a plugin-relative path so that the asset ships inside the package and follows the existing containment rules (§4.1); remote references are excluded to avoid load-time fetching, SSRF, and privacy concerns. Richer presentation data and client policy remain in client `extensions` (§8).
