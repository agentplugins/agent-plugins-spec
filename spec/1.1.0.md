...
The manifest MUST be JSON and MUST contain a top-level object. Its schema is closed: the only permitted top-level fields are `$schema`, `name`, `presentation`, `version`, `description`, `author`, `homepage`, `repository`, `license`, `keywords`, and `extensions`.
...
A non-object `extensions` field is handled as defined in §8.1. Every permitted field otherwise MUST match the type and constraints defined below. Any manifest violation other than an unknown top-level field, a non-object `extensions` field, or a string `presentation.icon` value that violates only the plugin-relative path requirements of §5.7.2 is fatal: the client MUST reject the plugin and MUST NOT discover or execute any of its components.
...
| Field         | Type     | Description                                                           |
| ------------- | -------- | --------------------------------------------------------------------- |
| `presentation` | object  | Passive presentation metadata for client UIs. See §5.7.               |
...
Except where this specification states an explicit constraint, metadata fields are validated only by their JSON types. Clients MUST NOT reject a manifest solely because `version` is not valid Semantic Versioning; `homepage`, `repository`, or `author.url` is not a recognized URL; `author.email` is not a recognized email address; `license` is not an SPDX identifier; or a string `presentation.icon` does not satisfy the plugin-relative path requirements of §5.7.2.
...
The optional `presentation` field contains passive metadata for presenting the plugin in client user interfaces. It MUST be an object and MAY contain only `displayName` and `icon`. An empty object is valid. Any other field or value type makes the manifest invalid, except for a string `icon` that does not satisfy the path requirements in §5.7.2.

The `presentation` object is not an extension point. Client-specific presentation data belongs under `extensions` as defined in §8.

#### 5.7.1 Display name

The optional `presentation.displayName` field is a human-readable label for the plugin. Unlike `name`, it is not constrained by §5.5 and MAY contain mixed case, spaces, and other characters.

If present, `presentation.displayName` MUST be a string. A client that displays a plugin label MAY use `presentation.displayName` instead of `name`. If `presentation.displayName` is absent, the client MUST use `name` for that label. A client SHOULD treat an empty `presentation.displayName` as absent and fall back to `name`.

#### 5.7.2 Icon

The optional `presentation.icon` field identifies an image that represents the plugin in client user interfaces. If present, `presentation.icon` MUST be a string.

The value is a plugin-relative path as defined in §4.1: it MUST begin with `./`, be resolved against the plugin root, and remain within the filesystem-resolved plugin root. Agent Plugins does not define remote (`http:` or `https:`) icon references, and a client MUST NOT retrieve a remote resource to resolve `presentation.icon`.

The image SHOULD be square and at least 512x512 pixels. Whether and how a client renders the icon is client-defined, and a client MAY ignore `presentation.icon` entirely.

`presentation.icon` is advisory. At the schema level, the only constraint on `presentation.icon` is that, if present, it MUST be a string; a non-string value is a fatal type violation under §5.2. A string value that does not satisfy the plugin-relative path requirements above is not a fatal violation: if `presentation.icon` is absent, is not a valid plugin-relative path, or resolves outside the plugin root, the client MUST ignore the field and MUST continue loading the plugin, and its value or validity MUST NOT affect component discovery or execution. A client that renders the icon MUST treat the referenced file as untrusted image data: it MUST NOT execute or otherwise interpret any active content the file may contain (for example, script embedded in an SVG), and rendering the icon MUST NOT cause any network request, including a request to resolve a subresource referenced by the file. Agent Plugins defines no portable default icon; any placeholder is client-defined.
...
2. An unknown top-level field, a non-object `extensions` field, or a string `presentation.icon` value that violates only the plugin-relative path requirements of §5.7.2 is non-fatal under §5.2, §8.1, and §5.7.2. Any other violation of the `plugin.json` requirements is fatal to the plugin: the client MUST reject the plugin and MUST NOT discover or execute any of its components.
...
- [ ] Validate the closed `presentation` object when present ([§5.7](#57-presentation))
- [ ] When displaying a plugin label, prefer `presentation.displayName` when present and non-empty, otherwise fall back to `name` ([§5.7.1](#571-display-name))
- [ ] Treat `presentation.icon` as advisory: ignore it unless it is a plugin-relative path that resolves within the plugin root, and never retrieve a remote resource to resolve it ([§5.7.2](#572-icon))
...
### Why portable presentation metadata?

A plugin's human-readable label and packaged visual identifier need to travel with the plugin for it to remain recognizable across clients. The constrained `name` identifier (§5.5) is unsuitable as a user-facing label, so `presentation.displayName` provides an unconstrained label with a defined fallback to `name`. `presentation.icon` provides an optional visual identifier. Both fields affect presentation only and never component discovery or execution.

The closed `presentation` object groups passive UI metadata without defining a general interface or extension namespace. The portable contract stops at identity. `presentation.icon` is a plugin-relative path so that the asset ships inside the package and follows the existing containment rules (§4.1); remote references are excluded to avoid load-time fetching, SSRF, and privacy concerns. Richer presentation data and client policy remain in client `extensions` (§8).
